---
title: Processing the BLUEPRINT dataset
author: Peter Hickey
...
# Adapted from https://www.bioconductor.org/packages/release/bioc/vignettes/DeepBlueR/inst/doc/DeepBlueR.html#large-scale-analysis-of-dna-methylation-across-212-samples-from-the-blueprint-consortium
...
blueprint_DNA_meth <- blueprint_DNA_meth[
  grep("\\.wig$", blueprint_DNA_meth[["name"]]), ]

# deepblue_info() returns a nested list for these queries.
blueprint_DNA_meth_info <- deepblue_info(id = blueprint_DNA_meth[["id"]])
...
# TODO: Separate Rmds for the other projects?
# TODO: BLUEPRINT HSC Differention calls it WGBS
# TODO: CEEHRC calls it WGBS
# TODO: DEEP (IHEC) calls it WGBS (also has NOME-seq)
# TODO: Roadmap Epigenomcs calls it WGBS (also has RRBS)


# blueprint_DNA_meth <- blueprint_DNA_meth[
#   grep("CPG_methylation_calls.bs_call",
#     deepblue_extract_names(blueprint_DNA_meth)),]
# 
# # TODO: Ask Stephanie for details on how these were selected.
# # "Oh, I was aiming to get all the purified whole blood cell types (e.g. CD4T, CD8T, NK, Bcell, Gran, Mono, etc) which matches this package: https://bioconductor.org/packages/release/data/experiment/html/FlowSorted.Blood.450k.html" (https://community-bioc.slack.com/archives/DE38USPDW/p1597668346001100)
...
stopifnot(nrow(unique(coldata)) == (nrow(coldata) / 2))
coldata <- unique(coldata)

# TODO: Need to match up each row of coldata to its pair of bigWig files in df. 
# TODO: THE CODE BELOW IS OLD AND DOESN'T WORK

files_df <- df[["extra_metadata"]][, "FILE", drop = FALSE]
rownames(files_df) <- df$sample_id
files_df[["type"]] <- ifelse(grepl("bs_cov", files_df[["FILE"]]), "cov", "call")
files_df[rownames(coldata), ]
...
# Add bs_cov and bs_call file names.
match(coldata[["sample_id"]], grep(
    "bs_cov",
    df[["extra_metadata"]][["FILE"]],
    value = TRUE))
...
call_prefix <- vapply(
  ...
  character(1), 
  1)
stopifnot(identical(cov_prefix, call_prefix))

cov_prefix_o <- order(cov_prefix)
call_prefix_o <- order(call_prefix)

stopifnot(
  identical(cov_prefix[cov_prefix_o], call_prefix[call_prefix_o]))
```
